...
export function ScorecardTab({ innings, matchId }: Props) {
  ...
  const [totals, setTotals] = useState({
    ...
    totalOversFaced: innings.totalOversFaced,
    extrasWides: innings.extrasWides,
    extrasNoBalls: innings.extrasNoBalls,
    extrasLegByes: innings.extrasLegByes,
    extrasByes: innings.extrasByes,
    extrasPenalty: innings.extrasPenalty,
  })

  // Batting form
  const [bat, setBat] = useState({
    playerId: '', playerName: '', battingStyle: '',
    battingPosition: innings.moraBatting?.length + 1 || 1,
    runs: 0, ballsFaced: 0, fours: 0, sixes: 0,
    isNotOut: false, minutesBatted: '',
    dismissalType: '', dismissedByOppBowlerName: '',
    dismissedByOppBowlerStyle: '', fieldedByOppName: '',
    dismissedByMoraBowlerId: '', fieldedByMoraPlayerId: '',
  })

  // Bowling form
  const [bowl, setBowl] = useState({
    playerId: '', playerName: '', bowlingStyle: '',
  ...
  const [fow, setFow] = useState({
    ...
    dismissedPlayerName: '',
    dismissedMoraPlayerId: '', dismissedOppPlayerId: '',
  })
  ...
  const saveTotals = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg('Totals saved.')
      refresh()
    } catch { setError('Failed to save totals.') }
  }
    ...
    try {
      if (isMora) {
        await inningsApi.addMoraBatting(innings.id, {
          ...
          minutesBatted:              bat.minutesBatted ? Number(bat.minutesBatted) : null,
      ...
      setBat(b => ({ ...b,
        ...
        battingPosition: Number(bat.battingPosition) + 1,
      }))
      refresh()
    } catch { setError('Failed to add batting line.') }
  }
  ...
  const saveBowling = async (e: React.FormEvent) => {
    ...
    try {
      ...
      refresh()
    } catch { setError('Failed to add bowling figures.') }
  }
  ...
  const saveFow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFow(f => ({
        ...f, wicketNumber: Number(f.wicketNumber) + 1,
        scoreAtFall: 0, overAtFall: '', dismissedPlayerName: '',
        dismissedMoraPlayerId: '',
      }))
      refresh()
    } catch { setError('Failed to add fall of wicket.') }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5">
        ...
        {innings.moraBatting?.length > 0 && (
          <div className="mb-4">
            ...
            <div className="space-y-1">
              {innings.moraBatting.map((b: any) => (
                <div key={b.id}
                  className="flex gap-3 text-sm bg-slate-900/40 rounded px-3 py-1.5">
                  <span className="text-slate-400 w-5">{b.battingPosition}</span>
                  <span className="flex-1 text-white">{b.playerFullName}</span>
                  <span className="text-slate-300 font-semibold">
                    {b.runs}{b.isNotOut ? '*' : ''}
                  </span>
                  <span className="text-slate-500">({b.ballsFaced})</span>
                  <span className="text-slate-500 text-xs">{b.dismissalType ?? 'not out'}</span>
                </div>
              ))}
              {innings.opponentBatting?.map((b: any) => (
                <div key={b.id}
                  className="flex gap-3 text-sm bg-slate-900/40 rounded px-3 py-1.5">
                  <span className="text-slate-400 w-5">{b.battingPosition}</span>
                  <span className="flex-1 text-white">{b.playerName}</span>
                  <span className="text-slate-300 font-semibold">
                    {b.runs}{b.isNotOut ? '*' : ''}
                  </span>
                  <span className="text-slate-500">({b.ballsFaced})</span>
        ...
        {/* Bowling summary */}
        {(innings.moraBowling?.length > 0 || innings.opponentBowling?.length > 0) && (
          <div>
            <p className="text-xs text-slate-400 mb-2 font-medium uppercase tracking-wide">
              Bowling
            </p>
            <div className="space-y-1">
              {(isMora ? innings.opponentBowling : innings.moraBowling)?.map((b: any) => (
                <div key={b.id}
                  className="flex gap-3 text-sm bg-slate-900/40 rounded px-3 py-1.5">
                  ...
                  <span className="text-white font-semibold">{b.wickets}w</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        {(['totals','batting','bowling','fow'] as ActiveForm[]).map(f => (
          <button
            key={f}
      ...
      {active === 'totals' && (
        <form onSubmit={saveTotals}
          ...
          <div className="grid grid-cols-4 gap-3">
            {(['totalRuns','totalWickets','totalOversFaced'] as const).map(k => (
              <FormField key={k} label={k.replace(/([A-Z])/g, ' $1').trim()}>
          ...
          <div className="grid grid-cols-5 gap-3">
            {(['extrasWides','extrasNoBalls','extrasLegByes','extrasByes','extrasPenalty'] as const)
              .map(k => (
              <FormField key={k} label={k.replace('extras','').trim()}>
        ...
        <form onSubmit={saveBatting}
          ...
          <div className="grid grid-cols-3 gap-4">
            {isMora ? (
              <FormField label="Batter" required>
                <select value={bat.playerId}
                  onChange={e => setBat(b => ({ ...b, playerId: e.target.value }))}
              ...
              <>
                <FormField label="Batter Name" required>
                  <input value={bat.playerName}
                    onChange={e => setBat(b => ({ ...b, playerName: e.target.value }))}
                    className="input-base w-full" required />
                </FormField>
                <FormField label="Batting Hand">
                  <select value={bat.battingStyle}
                    onChange={e => setBat(b => ({ ...b, battingStyle: e.target.value }))}
                    className="input-base w-full">
            ...
            <FormField label="Position">
              <input type="number" min={1} max={11}
                ...
                onChange={e => setBat(b => ({ ...b, battingPosition: Number(e.target.value) }))}
                className="input-base w-full" />
            </FormField>
          </div>
          ...
          <div className="grid grid-cols-5 gap-3">
            {(['runs','ballsFaced','fours','sixes'] as const).map(k => (
              <FormField key={k} label={k.replace(/([A-Z])/g, ' $1').trim()}>
                <input type="number" min={0}
                  value={(bat as any)[k]}
                  onChange={e => setBat(b => ({ ...b, [k]: e.target.value }))}
                  className="input-base w-full" />
              </FormField>
            ))}
            <FormField label="Mins">
              <input type="number" min={0}
                ...
                onChange={e => setBat(b => ({ ...b, minutesBatted: e.target.value }))}
          ...
          <div className="grid grid-cols-3 gap-4">
            <FormField label="Dismissal">
              <select value={bat.dismissalType}
                onChange={e => setBat(b => ({ ...b, dismissalType: e.target.value }))}
                className="input-base w-full">
            ...
            {isMora && bat.dismissalType && bat.dismissalType !== 'DNB' && (
              <>
                <FormField label="Bowler Name">
                  <input value={bat.dismissedByOppBowlerName}
                    onChange={e => setBat(b => ({
                      ...b, dismissedByOppBowlerName: e.target.value
                    }))}
                    className="input-base w-full" placeholder="Opponent bowler" />
                </FormField>
                <FormField label="Bowler Style">
                  <select value={bat.dismissedByOppBowlerStyle}
                    onChange={e => setBat(b => ({
                      ...b, dismissedByOppBowlerStyle: e.target.value
                    }))}
                    className="input-base w-full">
                    <option value="">Unknown</option>
                    {['RF','RFM','RM','OB','LB','SLA','SLO','LM','LF'].map(s => (
                      <option key={s} value={s}>{s}</option>
                    ))}
                  </select>
                </FormField>
                <FormField label="Fielder (if caught/run out)">
                  <input value={bat.fieldedByOppName}
                    onChange={e => setBat(b => ({ ...b, fieldedByOppName: e.target.value }))}
                    className="input-base w-full" placeholder="Fielder name" />
                </FormField>
              </>
            )}
            {!isMora && bat.dismissalType && bat.dismissalType !== 'DNB' && (
              <>
                <FormField label="Dismissed By (our bowler)">
                  <select value={bat.dismissedByMoraBowlerId}
                    onChange={e => setBat(b => ({
                ...
                <FormField label="Fielder (our player)">
                  <select value={bat.fieldedByMoraPlayerId}
                    onChange={e => setBat(b => ({
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={bat.isNotOut}
              onChange={e => setBat(b => ({ ...b, isNotOut: e.target.checked }))}
      ...
      {active === 'bowling' && (
        <form onSubmit={saveBowling}
          ...
          <div className="grid grid-cols-3 gap-4">
            {!isMora ? (
              <FormField label="Bowler" required>
                <select value={bowl.playerId}
                  onChange={e => setBowl(b => ({ ...b, playerId: e.target.value }))}
                  className="input-base w-full" required>
                  <option value="">Select bowler...</option>
                  {players?.map(p => (
                    <option key={p.id} value={p.id}>{p.fullName}</option>
              ...
              <>
                <FormField label="Bowler Name" required>
                  <input value={bowl.playerName}
                    onChange={e => setBowl(b => ({ ...b, playerName: e.target.value }))}
                    className="input-base w-full" required />
                </FormField>
                <FormField label="Bowling Style">
                  <select value={bowl.bowlingStyle}
                    onChange={e => setBowl(b => ({ ...b, bowlingStyle: e.target.value }))}
                    className="input-base w-full">
                    <option value="">Unknown</option>
                    {['RF','RFM','RM','RMF','OB','LB','SLA','SLO','LM','LMF','LF'].map(s => (
                      <option key={s} value={s}>{s}</option>
                    ))}
                  </select>
                </FormField>
          ...
          <div className="grid grid-cols-6 gap-3">
            {(['oversBowled','maidens','runsConceded','wickets','wides','noBalls'] as const)
              .map(k => (
              <FormField key={k} label={k.replace(/([A-Z])/g, ' $1').slice(0,8)}>
                <input type="number" min={0} step={k === 'oversBowled' ? '0.1' : '1'}
                  value={(bowl as any)[k]}
                  onChange={e => setBowl(b => ({ ...b, [k]: e.target.value }))}
                  className="input-base w-full" />
              </FormField>
            ))}
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            <FormField label="Wicket #">
              <input type="number" min={1} max={10}
                ...
                onChange={e => setFow(f => ({ ...f, wicketNumber: Number(e.target.value) }))}
                className="input-base w-full" />
            </FormField>
            <FormField label="Score at Fall">
              <input type="number" min={0}
                value={fow.scoreAtFall}
                onChange={e => setFow(f => ({ ...f, scoreAtFall: Number(e.target.value) }))}
                className="input-base w-full" />
            </FormField>
            <FormField label="Over" hint="e.g. 12.4">
              <input type="number" min={0} step="0.1"
                value={fow.overAtFall}
                onChange={e => setFow(f => ({ ...f, overAtFall: e.target.value }))}
                className="input-base w-full" />
            </FormField>
            <FormField label="Dismissed Player">
              {isMora ? (
                <select value={fow.dismissedMoraPlayerId}
                  onChange={e => {
                ...
                <input value={fow.dismissedPlayerName}
                  onChange={e => setFow(f => ({ ...f, dismissedPlayerName: e.target.value }))}
                  className="input-base w-full" placeholder="Player name" />
              )}
            </FormField>
          </div>
          <button type="submit"
