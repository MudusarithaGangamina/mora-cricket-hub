...
type Tab = 'overview' | 'squad' | 'innings1-scorecard' | 'innings2-scorecard'
         | 'innings1-delivery' | 'innings2-delivery' | 'setup'
...
export default function MatchEntryPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${match.matchDate} · ${match.venueName ?? 'Unknown venue'} · ${match.surfaceType}`}
        action={
      ...
      <div className={clsx(
        ...
        {match.moraCaptainName && (
          <div className="ml-auto text-right text-xs text-slate-400">
            <div>Captain: <span className="text-slate-300">{match.moraCaptainName}</span></div>
            {match.playerOfMatchName && (
              <div>POTM: <span className="text-slate-300">{match.playerOfMatchName}</span></div>
            )}
          </div>
        )}
      </div>
      ...
      {/* Tab content */}
      {tab === 'overview' && (
        <OverviewTab match={match} innings1={innings1} innings2={innings2} />
      )}
      {tab === 'squad' && (
  <SquadTab
    matchId={id!}
  ...
  />
      ...
      {tab === 'setup' && (
        <InningsSetupTab matchId={id!} existingInnings={scorecards ?? []} />
      )}
  ...
  <DeliveryEntryTab innings={innings1} matchId={id!} />
)}
{tab === 'innings2-delivery' && innings2 && (
  <DeliveryEntryTab innings={innings2} matchId={id!} />
)}
    </div>
  )
}

function OverviewTab({ match, innings1, innings2 }: {
  match: any; innings1: any; innings2: any
}) {
  return (
...
function InningsSummaryCard({ innings, label }: { innings: any; label: string }) {
  return (
